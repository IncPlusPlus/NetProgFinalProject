...
/**
 * This integration test ensures that when the server sends the disconnect signal, the clients and
 * slaves properly disconnect from it.
 */
class ClientsAbandonDyingServerIT {
  ...
  void oneClientOneSlave() throws IOException {
    performActionsAndAssertions(
        new Client("localhost", serverPort), new Slave("localhost", serverPort));
  }
        ...
        properClient -> {
          ...
          if (!properClient.isClosed()) {
            try {
              ...
              Thread.sleep(50);
              if (properClient.isClosed()) System.out.println("Success!!");
            } catch (InterruptedException e) {
              throw new IllegalStateException(
                  "Got interrupted while generously sleeping on a client.", e);
            }
          }
        ...
        properClient -> {
          ...
          if (server.isConnected(properClient.getConnectionId())) {
            try {
              ...
              throw new IllegalStateException(
                  "Got interrupted while generously sleeping on the connected entities map.", e);
            }
          }
        });
  }
