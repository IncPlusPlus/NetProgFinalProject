...
public class NormalIT {
  /**
   * This should always stay at zero. When at zero, the server will use whatever port is available.
   *
   * <p>For local testing, you can make this nonzero and have {@linkplain
   * io.github.incplusplus.peerprocessing.slave.PersistentSlaveRunner}s to help when you're running
   * the tests.
   */
  ...
  private static Stream<Arguments> provideMatrices() {
    ...
    for (int i = 0; i < numMatrices; i++) {
      matrixPairs.add(
          Arguments.of(
              new BigDecimalMatrix().random(aRows, aColsAndBRows),
  ...
  void mathQuerySingleSlave(String input, String expected)
  ...
  @ParameterizedTest
  @CsvFileSource(resources = "/SimpleMathProblems.csv", numLinesToSkip = 1)
  void mathQueryTwoSlaves(String input, String expected)
      throws IOException, ExecutionException, InterruptedException {
    FutureTask<BigDecimal> task;
    try (Client myClient = new Client("localhost", serverPort);
  ...
  @ParameterizedTest
  @MethodSource("provideMatrices")
  void matrixQueryManySlaves(BigDecimalMatrix matrix1, BigDecimalMatrix matrix2)
      throws IOException, ExecutionException, InterruptedException {
    FutureTask<BigDecimalMatrix> task;
    try (Client myClient = new Client("localhost", serverPort);
