...
public class Server {
  ...
  static final String serverName = "Processing Server";
   ...
   * Elements of this list exist while an incoming connection is established and are removed after
   * it has been established what {@linkplain MemberType} the incoming connection is.
   */
  ...
  /**
   * A HashMap that contains jobs that are to be processed in batches. ClientObj and SlaveObj should
   * not interact with this Map. Only the server coordinates when an action is taken with the items
   * in this Map.
   */
   ...
   * Start a server.
   *
   * @param serverPort the port to start the server on. If set to 0, the server will listen on
  ...
  public int start(int serverPort, boolean verbose) throws IOException {
    if (verbose) StupidSimpleLogger.enable();
          ...
          while (started.get()) {
            try {
              ...
              handlerThread.setName(
                  "ConnectionHandler thread for "
                      + incomingConnection.getLocalAddress().getHostAddress());
  ...
  /**
   * Register a {@linkplain ConnectedEntity} within the server. This keeps track of the connected
   * clients and slaves.
   *
  ...
  private void register(ConnectedEntity connectedEntity) {
    if (connectedEntity instanceof SlaveObj) {
      ...
      throw new IllegalArgumentException(
          "Argument 'connectedEntity' must be a SlaveObj or ClientObj.");
    }
  }
  // </editor-fold>
  ...
  /**
   * Remove the specified {@linkplain ConnectionHandler} from the server's list of
   * ConnectionHandlers.
   *
   ...
   * This can be a {@link io.github.incplusplus.peerprocessing.client.Client} or a {@link
   * io.github.incplusplus.peerprocessing.slave.Slave}
   *
   ...
   * @return whether or not the server is still connected to the slave or client of the specified
   *     UUID
   */
  ...
  /**
   * Add a query to the server. The server will then add this query to the query queue where it will
   * be removed and processed. The query will also be added to a map of jobs to determine the source
   * and destination of jobs received by slaves.
   *
  ...
  /**
   * Remove a query from the jobs list. At this point, the query has just been processed by a slave.
  ...
  /**
   * Tell the server a certain job has been completed. The server checks whether this Query is a
   * member of any active {@linkplain BatchQuery} instance.
   *
  ...
  private void relayToAppropriateClient(Query query) throws JsonProcessingException {
    ...
    if (requestSource == null) {
      error(
          "Tried to tell client "
              + query.getRequestingClientUUID()
              + " the answer to "
              + query.getQueryId()
              + " but the client disappeared!");
    } else {
      requestSource.acceptCompleted(query);
    }
  }

  private void sendToLeastBusySlave(Query job)
  ...
  class ClientObj extends ConnectedEntity {
    ClientObj(
      ...
      super(outToClient, inFromClient, socket, connectionUUID);
    }
          ...
          if (header.equals(QUERY)) {
            solve(
                SHARED_MAPPER.readValue(
                    Objects.requireNonNull(decode(lineFromClient)), Query.class));
          } else if (header.equals(IDENTIFY)) {
            getOutToClient()
                .println(
                    msg(
                        SHARED_MAPPER.writeValueAsString(provideIntroductionFromServer()),
                        IDENTITY));
  ...
  class SlaveObj extends ConnectedEntity {
        ...
        try {
          ...
          if (header.equals(RESULT)) {
            Query completedQuery =
                SHARED_MAPPER.readValue(Objects.requireNonNull(decode(lineFromSlave)), Query.class);
            ...
            boolean definitelyShouldStillPossessJob =
    ...
    /**
     * @return the list of UUIDs representing jobs that this slave is currently responsible for.
     *     Useful for recovering a job if a slave suddenly disconnects.
     */
  ...
  class ConnectionHandler implements Runnable {
    ...
    ConnectionHandler(Socket incomingConnection) throws IOException {
      if (incomingConnection == null || incomingConnection.isClosed())
        throw new IllegalArgumentException(
            "An incoming connection was established" + "but was then immediately dropped.");
      debug(
          "New connection at "
              + incomingConnection.getLocalAddress().getHostAddress()
              + ":"
