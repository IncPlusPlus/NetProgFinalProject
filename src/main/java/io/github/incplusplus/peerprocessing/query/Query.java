...
@JsonSubTypes({
  @JsonSubTypes.Type(AlgebraicQuery.class),
  @JsonSubTypes.Type(MatrixQuery.class),
  @JsonSubTypes.Type(VectorQuery.class),
  @JsonSubTypes.Type(BatchQuery.class)
})
public abstract class Query {
  ...
  @JsonTypeInfo(use = JsonTypeInfo.Id.CLASS, include = JsonTypeInfo.As.WRAPPER_ARRAY)
  @JsonSubTypes({
    @JsonSubTypes.Type(BigDecimalMatrix.class),
    @JsonSubTypes.Type(MatrixQuery.class),
    @JsonSubTypes.Type(VectorQuery.class),
    @JsonSubTypes.Type(BatchQuery.class)
  })
  ...
  /** Perform whatever action will lead to the completed flag being switched to true. */
  ...
  /**
   * Whether or not the request is completed. NOTE THAT THIS CAN MEAN THAT IT WAS COMPLETED
   * EXCEPTIONALLY. CHECK THAT {@link #getReasonIncomplete()} IS NULL BEFORE ASSUMING SUCCESS!
   *
