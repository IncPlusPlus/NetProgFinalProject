...
/**
 * An immutable matrix containing entries that extend T. Implementing classes define their own
...
public abstract class RealMatrix<T> extends Matrix<T> {
  ...
  /**
   * Get the vectors required to compute a matrix multiplication operation this * other.
   *
   * @param other the matrix to multiply this matrix by
   * @return a Quartet where the first BigDecimal[] is the first vector and the second BigDecimal[]
   *     is the second vector. The third item and fourth items (Integers) are the row and column
   *     index respectively that the result of this operation would be placed in when creating the
   *     product matrix.
   */
  public abstract List<Quartet<BigDecimal[], BigDecimal[], Integer, Integer>>
      getVectorsForMultiplyingWith(RealMatrix<T> other);
  ...
  /**
   * The fact that this is an instance method and not a static method is a byproduct of the desire
   * to keep the generic abstract in place. It doesn't matter what RealMatrix instance you use this
   * method on. The only things that are relevant to this operation are the two parameters.
   *
